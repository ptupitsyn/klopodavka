...
//---------------------------------------------------------------------------*
//
//    ResourcePathToImageSourceConverter.cs: Converts resource path to BitmapImage.
//
//---
//
...
{
   /// <summary>
   /// Converts resource path to BitmapImage.
   /// </summary>
   public class ResourcePathToImageSourceConverter : IMultiValueConverter
   {
      ...
      public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
      {
         ...
         var path = DisplayMemberToValueConverter.EvaluateExpression(values[0], values[1]) as string;
         if (string.IsNullOrEmpty(path)) return null;

         return new BitmapImage(new Uri(path));
         //Stream fileStream = values[0].GetType().Assembly.GetManifestResourceStream(path);
